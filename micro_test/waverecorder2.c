...
//  /* Reset the time base variable */
   ...
   Switch = 0;
//
  //while(HCD_IsDeviceConnected(&USB_OTG_Core))//jezeli podlaczony? gdzie jest zmienna usb_otg_core?
while(1)//petla do nagrywania
{
//    /* Wait for the recording time */
/*-------->*///if (Time_Rec_Base <= TIME_REC)//tej zmiennej tez nie ma
//    //{
...
//		ta czesc juz raczej nie potrzebna ram buffer to usb key ? raczej nie
...
//            buf_idx = 0;
//            /* Write the stored data in the RAM to the USB Key */
//            f_write (&file, (uint16_t*)RAM_Buf, RAM_BUFFER_SIZE*2 , (void *)&bytesWritten);//tutaj jest zapis do pliku ktory wczesniej wykomentowalem
//          }
//        }
//      //}
//
      /* User button pressed */
      if ( Command_index != 1)//po nacisnieciu przycisku stop i wyjscie z while'a
      {
        /* Stop recording */
        WaveRecorderStop();
        Command_index = 0;
